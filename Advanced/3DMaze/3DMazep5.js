...
let entry = {x:4, y:0} // currently just supports north wall entries ...
...
function draw() {
  ...
  if (visited < size_x * size_y && --steps > 0) {
    fill(0);
    circle(
      cell.col * blockSizeX + blockSizeX / 2,
      cell.row * blockSizeY + blockSizeY / 2,
      blockSizeX / 2
    );
    //nextStep();
  } else {
    ...
    for (let x = 0; x < size_x; x++) {
      for (let y = 0; y < size_y; y++) {
        // if there is no east wall: introduce wall from center of (x, y) to center of (x+1, y)
        if (!maze.get(x, y).hasWall("e")) {
          line(x * blockSizeX + blockSizeX / 2, y * blockSizeY + blockSizeY / 2,
            (x + 1) * blockSizeX + blockSizeX / 2,y * blockSizeY + blockSizeY / 2
          );
        }
        // if there is no east wall: introduce wall from center of (x, y) to center of (x, y+1)
        if (!maze.get(x, y).hasWall("s")) {
          line(x * blockSizeX + blockSizeX / 2, y * blockSizeY + blockSizeY / 2,
            x * blockSizeX + blockSizeX / 2, (y + 1) * blockSizeY + blockSizeY / 2
          );
        }
      }
    }
    // paint the half wall on the entry cell ..
    line(
